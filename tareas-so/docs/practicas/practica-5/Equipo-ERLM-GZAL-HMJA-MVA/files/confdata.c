...
void fill_default_options(options *my_options)
{
    ...
    my_options->portnum = 80;    /* port to listen on */
...
void process_cmdline(int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "hFd:l:c:m:p:wkto")) != EOF)
        switch(ch)
        {
...
int process_configfile(options *my_options)
{
    ...
    while ( (lp=fgets(line,CONFIG_FILE_LINELEN,conffile)))
    {
        ...
        case oLoglevel:
            s=parseint(opcode,expr,1,9,fn,linenum,&global_options->loglevel);
        ...
        case oPortnum:
            ...
            s=parseint(opcode,expr,1,65535,fn,linenum,&global_options->portnum);
            break;

        case oDnslookups:

            s=parsebool(opcode,expr,fn,linenum, &global_options->dnslookups);
            break;

        case oForegroundonly:

            s=parsebool(opcode,expr,fn,linenum, &global_options->foregroundonly);
            break;
        ...
        case oDumpcore:
            ...
            s=parsebool(opcode,expr,fn,linenum, &global_options->dumpcore);
            break;

        case oTerminate:

            s=parsebool(opcode,expr,fn,linenum, &global_options->terminate);
            break;
