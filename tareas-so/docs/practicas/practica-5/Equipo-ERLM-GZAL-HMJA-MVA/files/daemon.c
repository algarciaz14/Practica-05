...
#ifdef __WAIT_STATUS_DEFN
#define __WAIT_STATUS_DEFN_IS_DEFINED 1
#else
...

{
    ...
    log_start(); /* sets up logging on fd's 0,1 and 2, unless foregroundonly */
    ...
    if (global_options->foregroundonly == FALSE)
    {
        if (become_daemon() < 0)
        {
            LOG(1,("Can't become daemon, exiting\n"));
        }
